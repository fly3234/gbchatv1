...
async def addGlobal(ctx):
    if ctx.author.guild_permissions.administrator:
        if not guild_exists(ctx.guild.id):
            ...
            with open('servers.json', 'w') as f:
                json.dump(servers, f, indent=4)
            embed = discord.Embed(title="**Willkommen im GlobalChat von GB - GlobalChat Bot™**",
            ...
            embed.set_footer(text='Bitte beachte, dass im GlobalChat stets ein Slowmode von mindestens 5 Sekunden'
                                  ' gesetzt sein sollte und keine Befehle benutzt werden.')
...
async def sendAll(message: Message):
    ...
    icon_url = "https://media.giphy.com/media/VX7yEoXAFf8as/giphy.gif"
    ...
    links = '[Invite](https://discord.com/api/oauth2/authorize?client_id=678055302112739338&permissions=8&scope=bot) ║ '
...
bot.run('Njc4MDU1MzAyMTEyNzM5MzM4.XxIOKw.Wryhl8wt5JFQp2tNYrdpUESUX7c')
